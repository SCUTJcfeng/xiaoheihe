...
class HeyBoxClient:
    ...
    def simu_view_newses(self,idlist,limit=0):
        if limit > 0:
            print('批量模式开启，目标为[%d]篇文章' % limit)
        else:
            print('批量模式开启，目标为[%d]篇文章' % len(idlist))
        i = 0
        for item in idlist:
    ...
    def simu_view_like_newses(self, idlist, limit=10):
        print('批量模式开启，目标为[%d]篇文章' % limit)
        for item in idlist:
            self.like_news(item[0], item[1])
            time.sleep(2)
        print('执行完毕')
    ...
    def simu_like_follows(self,likelist,limit=-1):
        if limit > 0:
            print('批量模式开启，目标为[%d]条动态' % limit)
        else:
            print('批量模式开启，目标为[%d]条动态' % len(likelist))
        i = 1
        likedcount = 0
        errorcount = 0
        for item in likelist:
            ...
            if not item[2]:
                ...
                if status == True:
                    ...
                    errorcount+=1
                    if errorcount == 3:
                        print('连续多条动态点赞出错，可能点赞次数用尽，终止任务')
                        break
            else:
                print('已点赞，跳过')
                likedcount+=1
                if likedcount == 5:
                    print('连续多条动态已点赞，终止任务')
                    break
            i+=1
            limit-=1
            if limit == 0:
    ...
    def simu_follow_followers(self,followerlist,limit=-1):
        ...
        for item in followerlist:
            if (item[1] == 2):
                print('尝试关注新粉丝@[%s]' % item[0])
                self.follow_user(item[0])
                time.sleep(1)
            elif (item[1] == 0):
                print('尝试关注用户@[%s]' % item[0])
                self.follow_user(item[0])
            limit-=1
            if limit == 0:
                break
    ...
    def auto(self):
        ...
        self.sign()
        # 获取文章列表id
        idlist = self.get_news_list(limit=30)
        # 分享第一篇
        self.share(idlist[0][1])
        # 点赞10篇文章
        for item in idlist[:10]:
            self.like_news(item[0], item[1])
            time.sleep(2)
    ...
    # 有用
    def get_news_list(self, offset=0, limit=30):
        url = _NEWS_LIST_
        self.__flush_params()
        params = {
            'tag': -1,
        ...
        try:
            ...
            try:
                ...
                if not has_video:
                    ...
                    print('标题:%s [视频]' % title)
                print('作者:%s @%s [%s级]' % (author_name,author_id,author_level))
                print('点击:%s 点赞:%s 评论:%s' % (click,like_count,comment_count))
        ...
        try:
            ...
            try:
                ...
                for item in follow_list:
                    result.append((item['userid'],item['is_follow']))
                print('关注[%s] 粉丝[%s]' % (follow_num,fan_num))
                return(result)
    ...
    def followlist_filter(self,idlist,value=200):
        ...
        for userobj in idlist:
            try:
                ...
                print('过滤出错')
                print(e)
        print('过滤后共有[%d]个用户' % len(filteredlist))
        return(filteredlist)
    ...
    def sign(self):
        ...
        try:
            ...
            print('签到成功，连续[%s]天' % sign_in_streak)
            print('获得[%s]盒币,[%s]经验' % (sign_in_coin,sign_in_exp))
            return(True)
        except ValueError as e:
            print('签到出错')
            print(e)
    ...
    def get_task_stats(self):
        ...
        try:
            ...
            if wait != 0:
                print('任务完成度 %d/%d' % (task - wait,task))
                return(False)
            else:
                print('任务全部完成')
                return(True)
    ...
    def get_task_detail(self):
        ...
        try:
            ...
            return '签到%s|分享%s|点赞%s' % ('√'if task1 else '×','√'if task2 else '×','√'if task3 else '×')
    ...
    def get_my_data(self):
        ...
        try:
            ...
            return '盒币[%s],经验[%d/%d],连续签到[%s]天' % (coin, exp, max_exp, sign_in_streak)
        except Exception as e:
            print(e)
        return '获取任务详情出错'
    ...
    def get_user_profile(self,userid):
        ...
        try:
            ...
            print('昵称:%s @%s [%s级]' % (username,userid,level))
            print('关注[%s],粉丝[%d],获赞[%s]' % (follow_num,fan_num,awd_num))
